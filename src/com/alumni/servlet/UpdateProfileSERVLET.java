...
//-----------------------------------------------------------------------------------------------------------------------			
			...
			if("j".equals(update.substring(1))){
				RequestDispatcher r1 = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=25"); // Enter Job type
...
//-----------------------------------------------------------------------------------------------------------------------			
			...
			if("j".equals(update.substring(1))){
				RequestDispatcher r1 = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=26"); // Enter Industry name
				r1.forward(request, response);
				return;
			}
...
//-----------------------------------------------------------------------------------------------------------------------			
			...
			if("j".equals(update.substring(1))){
				RequestDispatcher r1 = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=27"); // Enter Funcation Area
				r1.forward(request, response);
				return;
			}
...
if((StringUtils.isEmptyOrWhitespaceOnly(password)||StringUtils.isNullOrEmpty(password))&&("".equals(conpassword)||null==(conpassword))){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=100"); // empty password flied 
	r.forward(request, response);
	return;
}else if(!password.equals(conpassword)){
	RequestDispatcher editprofile=request.getRequestDispatcher("EditProfileSERVLET?resultFlag=2"); // password not match
	editprofile.forward(request,response);
	return;
}else if("on".equals(checkotherhobbies)&& StringUtils.isEmptyOrWhitespaceOnly(other)||StringUtils.isNullOrEmpty(other)){							
 	RequestDispatcher editprofile=request.getRequestDispatcher("EditProfileSERVLET?resultFlag=4"); // check box on empty text box 
	editprofile.forward(request, response);	
	return;
}else if("null".equals(checkotherhobbies)||null==(checkotherhobbies)&&!"".equals(other)){
	RequestDispatcher r1 = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=3"); // not checked and written text box
	r1.forward(request, response);
	return;
}else if (StringUtils.isEmptyOrWhitespaceOnly(firstname)||StringUtils.isNullOrEmpty(firstname)){
	RequestDispatcher rs = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=5"); // empty first name 
	rs.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(middlename)||StringUtils.isNullOrEmpty(middlename)){
	RequestDispatcher rs = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=6"); // empty middle name
	rs.forward(request, response);
	return;
} else if(StringUtils.isEmptyOrWhitespaceOnly(lastname)||StringUtils.isNullOrEmpty(lastname)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=7"); // empty last name
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(Gen)||StringUtils.isNullOrEmpty(Gen)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=8"); // empty gender 
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(mobilenumber)||StringUtils.isNullOrEmpty(mobilenumber)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=9");// empty mobile
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(email)||StringUtils.isNullOrEmpty(email)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=10"); // empty email
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(add1)||StringUtils.isNullOrEmpty(add1)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=11"); // empty address line 1 
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(add2)||StringUtils.isNullOrEmpty(add2)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=12"); // empty address line 2 
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(degree1)||StringUtils.isNullOrEmpty(degree1)||"".equals(degree1)||null==(degree1)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=13"); // empty highest degree 1
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(clg)||StringUtils.isNullOrEmpty(clg)||"".equals(clg)||null==(clg)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=14"); // empty first clgg
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(branch2)||StringUtils.isNullOrEmpty(branch2)||"".equals(branch2)||null==(branch2)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=15"); // empty branch 2
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(passooutyear2)||StringUtils.isNullOrEmpty(passooutyear2)||"".equals(passooutyear2)||null==(passooutyear2)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=16"); // empty passout 2
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(clg2)||StringUtils.isNullOrEmpty(clg2)||"".equals(clg2)||null==(clg2)){
	RequestDispatcher r =request.getRequestDispatcher("EditProfileSERVLET?resultFlag=17"); // empty clg 2
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(jyear)||StringUtils.isNullOrEmpty(jyear)||"".equals(jyear)||null==(jyear)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=18"); // empty job year
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(jmonth)||StringUtils.isNullOrEmpty(jmonth)||"".equals(jmonth)||null==(jmonth)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=19"); // empty job month
	r.forward(request, response);
	return;
}else if(StringUtils.isEmptyOrWhitespaceOnly(keyskills)||StringUtils.isNullOrEmpty(keyskills)||"".equals(keyskills)||null==(keyskills)){
	RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=20"); // empty keyskills
	r.forward(request, response);
	return;
}
//----------------------------------- bean started------------------------------------	
...
//	 		final String path = getServletContext().getRealPath("/")+"images\\profile\\";
	 ...
	 if("".equals(fileName)||StringUtils.isEmptyOrWhitespaceOnly(fileName)||StringUtils.isNullOrEmpty(fileName)||null==fileName||"null".equals(fileName)){
		 bean.setPic(oldbean.getPic());
			    ...
			    InputStream filecontent = null;
		 		...
		 		if(jpg.equals("jpg") || jpeg.equals("jpeg") ||png.equals("png")||JPG.equals("JPG")||png.equals("PNG")||jpeg.equals("JPEG"))
				{
				 try {
					 ...
					 if(jpeg.equals("jpeg"))
					 {
				        ...
				        String imgpath="http://alumninetwork.datadoctorr.com/alumni/imageUpload/"+lb.getM_id()+"."+jpeg;
					 ...
					 if(png.equals("png"))
					 {
				        ...
				        String imgpath="http://alumninetwork.datadoctorr.com/alumni/imageUpload/"+lb.getM_id()+"."+png;
				        ...
				        RequestDispatcher r = request.getRequestDispatcher("EditProfileSERVLET?resultFlag=23"); // file not found
						r.forward(request, response);
						return;
				    }
		...
		RequestDispatcher editprofile=request.getRequestDispatcher("profileSERVLET?flag=21"); 	// successfully updated
		editprofile.forward(request, response);
		return;
		}
			...
			RequestDispatcher editprofile=request.getRequestDispatcher("EditProfileSERVLET?resultFlag=22"); //  try again 
			editprofile.forward(request, response);
			return;
		}	
	} //----------------------- else post method----------------
