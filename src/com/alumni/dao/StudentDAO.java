...
public class StudentDAO {
	public void reportStudent(RepotUserBEAN bean)throws SQLException {
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("INSERT INTO repoted_users(Repoted_UId,Repoter_UId,issu_msg)VALUES (?,?,?);");
	...
	public int checkReporterUser(RepotUserBEAN bean)throws SQLException {
		...
		try{
			con=ConnectionDAO.getConnection();
			PreparedStatement ps=con.prepareStatement("SELECT count(Repoter_UId) FROM repoted_users where Repoted_UId=?;");
			ps.setInt(1,bean.getReportedId());
	...
	public int checkReportedUser(RepotUserBEAN bean)throws SQLException {
		...
		try{
			con=ConnectionDAO.getConnection();
			PreparedStatement ps=con.prepareStatement("SELECT count(Repoted_UId) FROM repoted_users where Repoted_UId=?;");
			ps.setInt(1,bean.getReportedId());
			ResultSet rs = ps.executeQuery();
			if(rs.next()){
				return rs.getInt(1);
